...
def resmodule(last_layer,conv_counter,stage,level,n,outch,outCH,kernel,l,lr_m):
    if conv_counter == 1:
        n.tops['Mconv_reduce%d_stage%d_L%d' % (conv_counter, stage, level+1)] = L.Convolution(n.tops[last_layer[level]], kernel_size=1,
                                      num_output=outch, pad=0,
                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                      weight_filler=dict(type='gaussian', std=0.01),
                                      bias_filler=dict(type='constant'))
        res = 'Mconv_reduce%d_stage%d_L%d' % (conv_counter, stage, level+1)
    else:
        res = last_layer[level];
    ...
    convname = 'Mconv_res%d_stage%d_L%d' % (conv_counter, stage, level+1)
    n.tops[convname] = L.Convolution(n.tops[last_layer[level]], kernel_size=kernel[l],
                                      num_output=outCH[l], pad=int(math.floor(kernel[l]/2)),
                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                      weight_filler=dict(type='gaussian', std=0.01),
                                      bias_filler=dict(type='constant'))
    last_layer[level] = convname
    ReLUname = 'Mrelu_res%d_stage%d_L%d' % (conv_counter, stage, level+1)
    n.tops[ReLUname] = L.ReLU(n.tops[last_layer[level]], in_place=True)
    
    eltname = 'Melt_%d_stage%d_L%d' % (conv_counter,stage, level+1)
    n.tops[eltname] = L.Eltwise(n.tops[last_layer[level]], n.tops[res])
    last_layer[level] = eltname
    #ReLUname = 'Mrelu_elt%d_stage%d_L%d' % (conv_counter, stage, level+1)
    #n.tops[ReLUname] = L.ReLU(n.tops[last_layer[level]], in_place=True)
    ...
    for l in range(0, len(layername)):
        ...
        if layername[l] == 'C':
            ...
            if layername[l+1] != 'L':
                if(state == 'image'):
                    if(batchnorm == 1):
                        ...
                        batchnorm_name = 'bn%d_%d_CPM' % (conv_counter, local_counter)
                        scale_name = 'scale%d_%d_CPM' % (conv_counter, local_counter)
                        n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[0]], in_place= True,
                                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                                 batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name] = L.Scale(n.tops[last_layer[0]], in_place=True,
                    ...
                    if(batchnorm == 1):
                        ...
                        scale_name = 'Mscale%d_stage%d' % (conv_counter, stage)
                        n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[0]], in_place= True,
                                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                                 batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name] = L.Scale(n.tops[last_layer[0]], in_place=True,
            ...
            for level in range(0,2):
                ...
                resmodule(last_layer,conv_counter,stage,level,n,128,outCH,kernel,l,lr_m);

                if layername[l+1] != 'L2' and layername[l+1] != 'L3':
                    if(state == 'image'):
                        if(batchnorm == 1):

                            batchnorm_name = 'bn%d_%d_CPM_L%d' % (conv_counter, stage, level+1)
                            scale_name = 'scale%d_%d_CPM_L%d' % (conv_counter, stage, level+1)
                            n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[level]],in_place=True,
                                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                                 batch_norm_param=dict(use_global_stats=False))
                            n.tops[scale_name] = L.Scale(n.tops[last_layer[level]], in_place=True,
                                                         param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                                         scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                                         bias_filler=dict(type='constant', value=0.0),bias_term=True))                            
                        #ReLUname = 'relu%d_stage%d' % (conv_counter, stage)
                        ...
                        if(batchnorm == 1):
                            ...
                            scale_name = 'Mscale%d_stage%d_L%d' % (conv_counter, stage, level+1)
                            n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[level]],in_place=True,
                                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                                 batch_norm_param=dict(use_global_stats=False))
                            n.tops[scale_name] = L.Scale(n.tops[last_layer[level]], in_place=True,
        ...
        #depth l,inception module
        elif layername[l] == 'inception':
            pathCH = {0:[128,64,64],
                      1:[128,64,64,64,64],
                      2:[57]}
            for level in range(0,2):
            #two branch---do same for L1&L2
                last_layer_p = [last_layer[level] for x in range(3)]
                for path in range(0,3):
                #three path in Inception module
                    if path == 0:
                    #11(128)+17(64)+71(64)
                        lr_m = lr_mult_distro[2]
                        conv_reduce = 'Mconv_reduce%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        bn_reduce = 'Mbn_reduce%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        scale_reduce = 'Mscale_reduce%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        relu_reduce = 'Mrelu_reduce%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        
                        conv_name1 = 'Mconv%d_stage%d_1x%d_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        bn_name1 = 'Mbn%d_stage%d_1x%d_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        scale_name1 = 'Mscale%d_stage%d_1x%d_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        relu_name1 = 'Mrelu%d_stage%d_1x%d_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        
                        conv_name2 = 'Mconv%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        
                        n.tops[conv_reduce] = L.Convolution(n.tops[last_layer_p[path]], kernel_size = 1,
                                                              num_output=pathCH[path][0], pad=0,
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_reduce
                        
                        n.tops[bn_reduce] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                             param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                             batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_reduce] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                                                     param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                                     scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                                     bias_filler=dict(type='constant', value=0.0),bias_term=True))
                        n.tops[relu_reduce] = L.ReLU(n.tops[last_layer_p[path]], in_place=True)
                        
                        n.tops[conv_name1] = L.Convolution(n.tops[last_layer_p[path]], kernel_h=1,kernel_w=kernel[l],
                                                              num_output=pathCH[path][1], pad_h=0,pad_w=int(math.floor(kernel[l]/2)),
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_name1
                        
                        n.tops[bn_name1] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                             param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                             batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name1] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                                                     param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                                     scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                                     bias_filler=dict(type='constant', value=0.0),bias_term=True))
                        n.tops[relu_name1] = L.ReLU(n.tops[last_layer_p[path]], in_place=True)
                        
                        n.tops[conv_name2] = L.Convolution(n.tops[last_layer_p[path]], kernel_h=kernel[l],kernel_w=1,
                                                              num_output=pathCH[path][2],  pad_h=int(math.floor(kernel[l]/2)),pad_w=0,
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_name2
                        
                        batchnorm_name = 'Mbn%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        scale_name = 'Mscale%d_stage%d_L%d_p%d' % (conv_counter, stage, level+1, path+1)
                        ReLUname = 'Mrelu%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                 batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                                         param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                         scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                         bias_filler=dict(type='constant', value=0.0),bias_term=True))
                        n.tops[ReLUname] = L.ReLU(n.tops[last_layer_p[path]], in_place=True)
                        print ReLUname
                        
                    elif path == 1:
                    #11(128)+71(64)+17(64)+17(64)+71(64)
                        ...
                        conv_name2 = 'Mconv%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        bn_name2 = 'Mbn%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        scale_name2 = 'Mscale%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)
                        relu_name2 = 'Mrelu%d_stage%d_%dx1_L%d_p%d' % (conv_counter, stage, kernel[l],level+1, path+1)

                        conv_name3 = 'Mconv%d_stage%d_1x%d_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)
                        bn_name3 = 'Mbn%d_stage%d_1x%d_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)
                        scale_name3 = 'Mscale%d_stage%d_1x%d_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)
                        relu_name3 = 'Mrelu%d_stage%d_1x%d_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)

                        conv_name4 = 'Mconv%d_stage%d_%dx1_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)
                        ...
                        n.tops[conv_name2] = L.Convolution(n.tops[last_layer_p[path]], kernel_h=1,kernel_w=kernel[l],
                                                              num_output=pathCH[path][2], pad_h=0,pad_w=int(math.floor(kernel[l]/2)),
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_name2
                        
                        n.tops[bn_name2] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                             param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                             batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name2] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                                                     param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                                     scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                                     bias_filler=dict(type='constant', value=0.0),bias_term=True))
                        n.tops[relu_name2] = L.ReLU(n.tops[last_layer_p[path]], in_place=True)

                        n.tops[conv_name3] = L.Convolution(n.tops[last_layer_p[path]], kernel_h=1,kernel_w=kernel[l],
                                                              num_output=pathCH[path][3], pad_h=0,pad_w=int(math.floor(kernel[l]/2)),
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_name3
                        
                        n.tops[bn_name3] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                             param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                             batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name3] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                                                     param=[dict(lr_mult=1.0,decay_mult=1.0),dict(lr_mult=1.0,decay_mult=0)],
                                                     scale_param=dict(axis=1,num_axes=1,filler=dict(type='constant', value=1.0),
                                                     bias_filler=dict(type='constant', value=0.0),bias_term=True))
                        n.tops[relu_name3] = L.ReLU(n.tops[last_layer_p[path]], in_place=True)

                        n.tops[conv_name4] = L.Convolution(n.tops[last_layer_p[path]], kernel_h=kernel[l],kernel_w=1,
                                                              num_output=pathCH[path][4],  pad_h=int(math.floor(kernel[l]/2)),pad_w=0,
                                                              param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                              weight_filler=dict(type='gaussian', std=0.01),
                                                              bias_filler=dict(type='constant'))
                        last_layer_p[path] = conv_name4
                        
                        batchnorm_name = 'Mbn%d_stage%d_L%d_p%d_2' % (conv_counter, stage, level+1, path+1)
                        scale_name = 'Mscale%d_stage%d_L%d_p%d_2' % (conv_counter, stage, level+1, path+1)
                        ReLUname = 'Mrelu%d_stage%d_%dx1_L%d_p%d_2' % (conv_counter, stage, kernel[l],level+1, path+1)
                        n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer_p[path]],in_place=True,
                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                 batch_norm_param=dict(use_global_stats=False))
                        n.tops[scale_name] = L.Scale(n.tops[last_layer_p[path]], in_place=True,
                ...
                #concatlayer 0,1,2 ==> 185
                Mconcat = 'concat_inception%d_stage%d_L%d' % (conv_counter, stage, level+1)
                n.tops[Mconcat] = L.Concat(n.tops[last_layer_p[0]], n.tops[last_layer_p[1]], n.tops[last_layer_p[2]], concat_param=dict(axis=1))
                last_layer[level] = Mconcat 
            conv_counter += 1
            local_counter += 1
        ...
        #Depth = l
            ...
            for level in range(0,2):
                if state == 'image':
                    ...
                    bn_name1 = 'Mbn%d_stage%d_1x%d_L%d' % (conv_counter, stage, kernel[l],level+1)
                    scale_name1 = 'Mscale%d_stage%d_1x%d_L%d' % (conv_counter, stage, kernel[l],level+1)
                if layername[l+1] == 'L2' or layername[l+1] == 'L3':
                    if level == 0:
                        outCH[l] = 38
                    else:
                ...
                n.tops[conv_name1] = L.Convolution(n.tops[last_layer[level]], kernel_h=1,kernel_w=kernel[l],
                                                      num_output=outCH[l], pad_h=0,pad_w=int(math.floor(kernel[l]/2)),
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))
                ...
                n.tops[conv_name2] = L.Convolution(n.tops[last_layer[level]], kernel_h=kernel[l],kernel_w=1,
                                                      num_output=outCH[l],  pad_h=int(math.floor(kernel[l]/2)),pad_w=0,
                                                      param=[dict(lr_mult=lr_m, decay_mult=1), dict(lr_mult=lr_m*2, decay_mult=0)],
                                                      weight_filler=dict(type='gaussian', std=0.01),
                                                      bias_filler=dict(type='constant'))
                last_layer[level] = conv_name2
            ...
            for level in range(0,2):
                if state == 'image':
                    ...
                    batchnorm_name = 'Mbn%d_stage%d_1x7_L%d' % (conv_counter, stage, level+1)
                    scale_name = 'Mscale%d_stage%d_1x7_L%d' % (conv_counter, stage, level+1)
                if layername[l+1] == 'L2' or layername[l+1] == 'L3':
                    if level == 0:
                        outCH[l] = 38
                    else:
                ...
                if layername[l+1] != 'L2' and layername[l+1] != 'L3':
                    if(state == 'image'):
                        ...
                        if(batchnorm == 1):
                            batchnorm_name = 'Mbn%d_stage%d_7x1_L%d' % (conv_counter, stage, level+1)
                            scale_name = 'Mscale%d_stage%d_7x1_L%d' % (conv_counter, stage, level+1)
                            n.tops[batchnorm_name] = L.BatchNorm(n.tops[last_layer[level]],in_place=True,
                                                                 param=[dict(lr_mult=0), dict(lr_mult=0), dict(lr_mult=0)],
                                                                 batch_norm_param=dict(use_global_stats=False))
                            n.tops[scale_name] = L.Scale(n.tops[last_layer[level]], in_place=True,
            ...
            concat1 = 'concat_stage%d_L1' % stage
            concat2 = 'concat_stage%d_L2' % stage
            n.tops[concat1] = L.Concat(n.tops[last_layer[0]], n.tops[share_point], concat_param=dict(axis=1))
            n.tops[concat2] = L.Concat(n.tops[last_layer[1]], n.tops[share_point], concat_param=dict(axis=1))
            
            local_counter = 1
            state = 'fuse'
            last_layer[0] = concat1
            last_layer[1] = concat2
            print last_layer
        elif layername[l] == '$':
...
%s/build/tools/caffe train --solver=pose_solver.prototxt --gpu all \
--weights=/home/vis/sunguanxiong/code/Realtime/model/_trained_COCO/default/stage1/pose_iter_440000.caffemodel \
...
if __name__ == "__main__":
    ...
    if(exp == 1):
        directory = 'COCO_exp_caffe/pose56/stage3_independent_res/'
        serverFolder = '/home/vis/sunguanxiong/code/Realtime/training/COCO_kpt/pose56/exp22'
        base_folder = '/home/vis/sunguanxiong/code/Realtime/training/'+directory+'model'
        dataFolder = '/home/vis/sunguanxiong/code/Realtime/training/COCO_kpt/lmdb'
        source = '/home/vis/sunguanxiong/code/Realtime/training/COCO_kpt/lmdb'
        base_lr = 4e-5   # 2e-5
        batch_size = 10
        np = 56    # num_parts
        ...
        for nc in range(0,1):
            ...
            outCH =     [64]*3 + [128]* 3  +  [256] * 4 + [256]  +  [512] * 2 + [256] + [128] + [ 0 ] + [128] * 3 + [512] +[np*2] + [ 0 ]
            stride =    [ 1 , 1,  2 ] * 2  +  [ 1 ] * 4 + [ 2 ]  +  [ 1 ] * 2 + [ 1 ] * 2     + [ 0 ] + [ 1 ] * 3  + [ 1 ] * 2     + [ 0 ] # first-stage

            #if stage >= 2:
            for s in range(2, stage+1):
                layername += ['@'] + ['res']* 5 + ['C2']*2  +  ['L2']
                kernel +=    [ 0 ] + [ 7 ] * 5  + [1,1]     +  [ 0 ]
